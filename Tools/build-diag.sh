#!/usr/bin/env bash
#
# Builds the temporary FlightPlan UI diagnostic mod (Tools/diag/DiagPlugin.cs) into
# Deploy/FlightPlanDiag/FlightPlanDiag.dll, ready to copy to $KSP2/mods/FlightPlanDiag/.
#
# It is NOT part of FlightPlan.dll and is never shipped. It exists so a single game launch can
...
OUT_DIR="$REPO_ROOT/Deploy/FlightPlanDiag"
OUT_DLL="$OUT_DIR/FlightPlanDiag.dll"
...
# ReduxLib, and compiling against the complete set keeps the binding identical to FlightPlan.dll's.
...
{
  "spec": "2.0",
  "mod_id": "FlightPlanDiag",
  "name": "Flight Plan UI Diagnostic",
  "author": "local",
  ...
  "conflicts": [],
  "main_assembly": "FlightPlanDiag.dll"
}
EOF

mkdir -p "$OUT_DIR/variants"
log "Wrote $OUT_DLL ($(stat -c %s "$OUT_DLL") bytes)"
log "Payload: $OUT_DIR (swinfo.json + FlightPlanDiag.dll + variants/)"
log ""
log "Deploy with:"
log "  cp -f \"$OUT_DLL\" \"\$KSP2/mods/FlightPlanDiag/\""
