...
namespace FlightPlan
{
    ...
    internal class BurnTimeOption
    {
        ...
        // Computed per call (the P2 rule: never capture the game instance at type-load).
        private static GameInstance Game => FlightPlanPlugin.Game;

        // The three selections this class reads all live on the controller, which is where the UI
        // writes them (the BurnOptionsDropdown callback and the two text fields in InitializeElements).
        // They were staged here under the legacy names while the controller did not compile; the
        // staging is gone, so there is exactly one owner per value again.
        ...
        public void SetBurnTime()
        {
            ...
            IKeplerPatch Orbit = Plugin._activeVessel.Orbit;
            ...
            if (currentTarget != null)
            {
                ...
                {
                    tgtOrbit = currentTarget.Part.PartOwner.SimulationObject.Orbit;
                }
                else if (currentTarget.IsVessel || currentTarget.IsCelestialBody)
                {
                    tgtOrbit = currentTarget.Orbit;
                }
            }
        ...
        // KEPT (commented, as in the legacy source) because it is the only record of which TimeReference
        // each maneuver offers - P5 drives the dropdown from Options.
