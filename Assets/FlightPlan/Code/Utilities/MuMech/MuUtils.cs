...
namespace MuMech
{
    ...
    {
        // NOTE (FlightPlan port, P2): NodeManager opened this class with a type-load-time capture of
        // the live game instance - a `static readonly` field of type `GameInstance`, assigned from
        // GameManager.Instance. That is a hard crash when the loader first touches this type before
        // the game instance exists. The game handle is now read at call time through MuMechRuntime;
        // the value each caller sees is unchanged. (The literal line is recorded in
        // Deploy/obj/PORT-PROGRESS.md, P2 block.)
        // The usings System/System.Collections.Generic above replace the legacy project's
        // <ImplicitUsings>true</ImplicitUsings>, which this project does not enable.
        ...
        {
            PatchedConicsOrbit ret = new PatchedConicsOrbit(MuMechRuntime.UniverseModel);
